...
class AboutBlocks < Neo::Koan
  ...
  def test_lambda_vs_blocks_vs_proc
    ...
    assert_equal "This is a test".length, test_string_lambda.call.length

    test_num_lambda = lambda { |n| n * 9 }
    assert_equal 3*9, test_num_lambda.call(3)

    changed_value = 30
    method_block { changed_value += 10 }
    assert_equal 30+10, changed_value
    
    p = Proc.new { |n| n * 3 }
    assert_equal 3*2, p.call(2)
  end
end
